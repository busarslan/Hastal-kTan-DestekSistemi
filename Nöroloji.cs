...
{
    public partial class Nöroloji : Form
    {
        ...
        public Nöroloji()
        {
            ...
            this.Load += new EventHandler(Nöroloji_Load); // Form yüklenirken çağrılacak olayı ekle
            this.ClientSize = new Size(800, 450); // Başlangıç boyutlarını ayarla
            this.Resize += new EventHandler(Nöroloji_Resize); // Form yeniden boyutlandırıldığında çağrılacak olayı ekle
        ...
        private void Nöroloji_Load(object sender, EventArgs e)
        {
            try
            {
        ...
        private void Nöroloji_Resize(object sender, EventArgs e)
        {
            ResizeControls();
            RepositionControls();
            CustomMenu.AddMenu(this);
            ...
            using (OleDbConnection conn = new OleDbConnection(connectionString))
            {
                try
                {
                    ...
                    using (OleDbCommand command = new OleDbCommand(query, conn))
                    {
                        ...
                        command.Parameters.AddWithValue("@uzmanlık", "Nöroloji");
        ...
        private string DetermineDiseases()
        {
            var symptoms = new Dictionary<string, bool>
            {
                ...
                { "Zayıflık His", chkWeakness.Checked },
                { "Titreme", chkTremors.Checked },
                { "Nöbet", chkSeizures.Checked },
                { "Uyuşukluk", chkNumbness.Checked },
                { "Yürüme Zorluğu", chkDifficultyWalking.Checked },
                { "Hafıza Kaybı", chkMemoryLoss.Checked },
                { "Görme Sorunları", chkVisionProblems.Checked }
            };

            // Nöroloji ile ilişkilendirilen hastalıklar ve semptomlar
            var diseases = new Dictionary<string, List<string>>
            {
                { "Migren", new List<string> { "Baş Ağrısı" } },
                { "Epilepsi", new List<string> { "Nöbet" } },
                { "Parkinson Hastalığı", new List<string> { "Titreme", "Yürüme Zorluğu" } },
                { "Multiple Skleroz", new List<string> { "Zayıflık His", "Uyuşukluk" } },
                { "Demans", new List<string> { "Hafıza Kaybı" } },
                { "Görme Sinirinin Optik Nöropatisi", new List<string> { "Görme Sorunları" } }
            };
