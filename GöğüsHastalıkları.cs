...
{
    public partial class GöğüsHastalıkları : Form
    {
        ...
        public GöğüsHastalıkları()
        {
            ...
            this.Load += new EventHandler(GöğüsHastalıkları_Load); // Form yüklenirken çağrılacak olayı ekle
            this.ClientSize = new Size(800, 450); // Başlangıç boyutlarını ayarla
            this.Resize += new EventHandler(GöğüsHastalıkları_Resize); // Form yeniden boyutlandırıldığında çağrılacak olayı ekle
            InitializeLabel1();  // alt ana başlık (kulak burun boğaz) 
            this.userTC = UserInfo.TCNo;

        }

        private void GöğüsHastalıkları_Load(object sender, EventArgs e)
        {
            try {
        ...
        private void GöğüsHastalıkları_Resize(object sender, EventArgs e)
        {
            ResizeControls();
            RepositionControls();
            CustomMenu.AddMenu(this);
        ...
        {
            baslik = new Label();
            baslik.Text = "Göğüs Hastalıkları";
        ...
        private void RepositionControls()
        {
            ...
            int checkBoxLeft = (this.ClientSize.Width - chkCough.Width) / 2;
            ...
            using (OleDbConnection conn = new OleDbConnection(connectionString))
            {
                try
                {
                    ...
                    using (OleDbCommand command = new OleDbCommand(query, conn))
                    {
                        ...
                        command.Parameters.AddWithValue("@uzmanlık", "GöğüsHastalıkları");
        ...
        private string DetermineDiseases()
        {
            var symptoms = new Dictionary<string, bool>
            {
                ...
                { "Göğüs Ağrısı", chkChestPain.Checked },
                { "Nefes Darlığı", chkShortnessOfBreath.Checked },
                { "Hırıltı", chkWheezing.Checked },
                { "Balgam", chkSputum.Checked },
                { "Yorgunluk", chkFatigue.Checked }
            };

            var diseases = new Dictionary<string, List<string>>
            {
                { "Akut Bronşit", new List<string> { "Öksürük", "Balgam" } },
                { "Kronik Bronşit", new List<string> { "Öksürük", "Balgam" } },
                { "Pnömoni", new List<string> { "Öksürük", "Balgam", "Nefes Darlığı", "Yorgunluk" } },
                { "Astım", new List<string> { "Öksürük", "Nefes Darlığı", "Hırıltı" } },
                { "KOAH", new List<string> { "Öksürük", "Nefes Darlığı", "Hırıltı", "Balgam", "Yorgunluk" } },
                { "Angina Pektoris", new List<string> { "Göğüs Ağrısı" } },
                { "Miyokard Enfarktüsü", new List<string> { "Göğüs Ağrısı", "Yorgunluk" } },
                { "Perikardit", new List<string> { "Göğüs Ağrısı" } },
                { "Kostokondrit", new List<string> { "Göğüs Ağrısı" } },
                { "Akciğer Kanseri", new List<string> { "Göğüs Ağrısı", "Öksürük", "Balgam", "Nefes Darlığı", "Yorgunluk" } }
            };

            var selectedSymptoms = symptoms.Where(s => s.Value).Select(s => s.Key).ToList();
            if (selectedSymptoms.Count < 1)
            {
                return "En az bir semptom seçin.";
            }
            ...
            {
                return "Belirtiler belirli bir hastalıkla eşleşmiyor.";
            }
        }
    }
}