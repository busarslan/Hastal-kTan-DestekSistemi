...
{
    public partial class EnfeksiyonHastalıkları : Form
    {
        ...
        public EnfeksiyonHastalıkları()
        {
            ...
            this.Load += new EventHandler(EnfeksiyonHastalıkları_Load); // Form yüklenirken çağrılacak olayı ekle
            this.ClientSize = new Size(800, 450); // Başlangıç boyutlarını ayarla
            this.Resize += new EventHandler(EnfeksiyonHastalıkları_Resize); // Form yeniden boyutlandırıldığında çağrılacak olayı ekle
        ...
        private void EnfeksiyonHastalıkları_Load(object sender, EventArgs e)
        {
            try {
        ...
        private void EnfeksiyonHastalıkları_Resize(object sender, EventArgs e)
        {
            ResizeControls();
            RepositionControls();
            CustomMenu.AddMenu(this);
        ...
        {
            baslik = new Label();
            baslik.Text = "Enfeksiyon Hastalıkları";
            ...
            using (OleDbConnection conn = new OleDbConnection(connectionString))
            {
                try
                {
                    ...
                    using (OleDbCommand command = new OleDbCommand(query, conn))
                    {
                        ...
                        command.Parameters.AddWithValue("@uzmanlık", "EnfeksiyonHastalıkları");
        ...
        private string DetermineDiseases()
        {
            var symptoms = new Dictionary<string, bool>
            {
                ...
                { "Kas ağrıları", chkMuscleAches.Checked },
                ...
                { "Mide bulantısı ve kusma", chkNausea.Checked },
                { "Karın ağrısı", chkAbdominalPain.Checked },
                { "Döküntü", chkRash.Checked },
                { "Titreme", chkChills.Checked },
                { "Lenf bezi şişmesi", chkLymphNodeSwelling.Checked }
            };

            var diseases = new Dictionary<string, List<string>>
            {
                { "Grip", new List<string> { "Ateş", "Yorgunluk", "Baş ağrısı", "Kas ağrıları", "Boğaz ağrısı", "Öksürük" } },
                { "Pnömoni", new List<string> { "Ateş", "Öksürük", "Nefes darlığı", "Titreme" } },
                { "Gastroenterit", new List<string> { "İshal", "Mide bulantısı ve kusma", "Karın ağrısı", "Ateş" } },
                { "Mononükleoz", new List<string> { "Ateş", "Boğaz ağrısı", "Lenf bezi şişmesi", "Yorgunluk" } },
                { "COVID-19", new List<string> { "Ateş", "Öksürük", "Nefes darlığı", "Yorgunluk", "Baş ağrısı", "Tat ve koku kaybı", "Kas ağrıları" } },
                { "Kızamık", new List<string> { "Ateş", "Döküntü", "Öksürük", "Gözlerde kızarıklık", "Burun akıntısı" } }
            };
