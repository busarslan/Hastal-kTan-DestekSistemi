...
{
    public partial class GöğüsCerrahi : Form
    {
        ...
        public GöğüsCerrahi()
        {
            ...
            this.Load += new EventHandler(GöğüsCerrahi_Load); // Form yüklenirken çağrılacak olayı ekle
            this.ClientSize = new Size(800, 450); // Başlangıç boyutlarını ayarla
            this.Resize += new EventHandler(GöğüsCerrahi_Resize); // Form yeniden boyutlandırıldığında çağrılacak olayı ekle
        ...
        private void GöğüsCerrahi_Load(object sender, EventArgs e)
        {
            try {
                ResizeControls();
                RepositionControls();
        ...
        private void GöğüsCerrahi_Resize(object sender, EventArgs e)
        {
            ResizeControls();
            RepositionControls();
            CustomMenu.AddMenu(this);
        ...
        private void RepositionControls()
        {
            ...
            int checkBoxLeft = (this.ClientSize.Width - chkChestPain.Width) / 2;
            ...
            using (OleDbConnection conn = new OleDbConnection(connectionString))
            {
                try
                {
                    ...
                    using (OleDbCommand command = new OleDbCommand(query, conn))
                    {
                        ...
                        command.Parameters.AddWithValue("@uzmanlık", "GöğüsCerrahi");
        ...
        private string DetermineDiseases()
        {
            var symptoms = new Dictionary<string, bool>
            {
                ...
                { "Göğüs Kitleleri", chkChestMasses.Checked },
                { "Göğüs Duvarı Deformiteleri", chkChestWallDeformities.Checked },
                { "Göğüs Travmaları", chkChestTrauma.Checked },
                { "Göğüs Duvarı Ağrısı", chkChestWallPain.Checked },
                { "Göğüs Duvarı Enfeksiyonları", chkChestWallInfections.Checked },
                { "Göğüs Duvarı Lezyonları", chkChestWallLesions.Checked },
                { "Göğüs Duvarı Tümörleri", chkChestWallTumors.Checked }
            };

            var diseases = new Dictionary<string, List<string>>
            {
                { "Plevra Effüzyonu", new List<string> { "Nefes Darlığı", "Öksürük", "Göğüs Duvarı Ağrısı" } },
                { "Pnömotoraks", new List<string> { "Nefes Darlığı", "Göğüs Duvarı Ağrısı" } },
                { "Pnömoni", new List<string> { "Öksürük", "Nefes Darlığı", "Göğüs Duvarı Ağrısı" } },
                { "Göğüs Duvarı Lezyonları", new List<string> { "Göğüs Kitleleri", "Göğüs Duvarı Deformiteleri", "Göğüs Duvarı Ağrısı" } }
            };
            ...
            {
                return "Belirtililer yetersiz, lütfen bir doktora danışın.";
            }
        }
    }
}