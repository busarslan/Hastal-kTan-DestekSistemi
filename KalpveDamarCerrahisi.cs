...
{
    public partial class KalpveDamarCerrahisi : Form
    {
        ...
        public KalpveDamarCerrahisi()
        {
            ...
            this.Load += new EventHandler(KalpveDamarCerrahisi_Load); // Form yüklenirken çağrılacak olayı ekle
            this.ClientSize = new Size(800, 450); // Başlangıç boyutlarını ayarla
            this.Resize += new EventHandler(KalpveDamarCerrahisi_Resize); // Form yeniden boyutlandırıldığında çağrılacak olayı ekle
            InitializeLabel1();  // alt ana başlık (kulak burun boğaz) 
            this.userTC = UserInfo.TCNo;
        }

        private void KalpveDamarCerrahisi_Load(object sender, EventArgs e)
        {
            try
            {
                ResizeControls();
        ...
        private void KalpveDamarCerrahisi_Resize(object sender, EventArgs e)
        {
            ResizeControls();
            RepositionControls();
            CustomMenu.AddMenu(this);
        ...
        {
            baslik = new Label();
            baslik.Text = "Kalp ve Damar Cerrahisi";
            ...
            using (OleDbConnection conn = new OleDbConnection(connectionString))
            {
                try
                {
                    ...
                    using (OleDbCommand command = new OleDbCommand(query, conn))
                    {
                        ...
                        command.Parameters.AddWithValue("@uzmanlık", "Kalp ve Damar Cerrahisi");
        ...
        private string DetermineDiseases()
        {
            var symptoms = new Dictionary<string, bool>
            {
                ...
                { "Bacak Ağrısı", chkLegPain.Checked },
                { "Ayak Şişmesi", chkFootSwelling.Checked },
                { "Düzensiz Nabız", chkIrregularPulse.Checked },
                { "Baş Dönmesi", chkDizziness.Checked },
                { "Bayılma", chkFainting.Checked }
            };

            var diseases = new Dictionary<string, List<string>>
            {
                { "Koroner Arter Hastalığı", new List<string> { "Göğüs Ağrısı", "Nefes Darlığı", "Halsizlik" } },
                { "Kalp Yetmezliği", new List<string> { "Nefes Darlığı", "Yorgunluk", "Kalp Çarpıntısı" } },
                { "Damar Tıkanıklığı (Tromboz)", new List<string> { "Bacak Ağrısı", "Ayak Şişmesi", "Düzensiz Nabız" } },
                { "Hipertansiyon", new List<string> { "Baş Dönmesi", "Bayılma" } }
            };
