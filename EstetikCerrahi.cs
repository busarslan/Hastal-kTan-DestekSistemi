...
{
    public partial class EstetikCerrahi : Form
    {
        ...
        public EstetikCerrahi()
        {
            ...
            this.Load += new EventHandler(EstetikCerrahi_Load); // Form yüklenirken çağrılacak olayı ekle
            this.ClientSize = new Size(800, 450); // Başlangıç boyutlarını ayarla
            this.Resize += new EventHandler(EstetikCerrahi_Resize); // Form yeniden boyutlandırıldığında çağrılacak olayı ekle
        ...
        private void EstetikCerrahi_Load(object sender, EventArgs e)
        {
            try { RepositionControls(); } // Form yüklenirken kontrolleri yeniden konumlandır
            catch { }
        }

        private void EstetikCerrahi_Resize(object sender, EventArgs e)
        {
            RepositionControls();
            ...
            using (OleDbConnection conn = new OleDbConnection(connectionString))
            {
                try
                {
                    ...
                    using (OleDbCommand command = new OleDbCommand(query, conn))
                    {
                        ...
                        command.Parameters.AddWithValue("@uzmanlık", "EstetikCerrahi");
        ...
        private string DetermineDiseases()
        {
            var symptoms = new Dictionary<string, bool>
            {
                ...
                { "İltihap", chkInflammation.Checked },
                { "Morarma", chkBruising.Checked },
                { "Yaralanma", chkInjury.Checked },
                { "Cilt Lekeleri", chkSkinLesions.Checked },
                { "Asimetri", chkAsymmetry.Checked },
                { "Kırık", chkFracture.Checked },
                { "Deri Döküntüsü", chkSkinRash.Checked },
                { "Yanık", chkBurn.Checked },
                { "Cilt Kanseri Belirtileri", chkSkinCancerSigns.Checked }
            };

            var diseases = new Dictionary<string, List<string>>
            {
                { "Yüz Travması", new List<string> { "Şişlik", "Ağrı", "Yaralanma", "Morarma", "Kırık" } },
                { "Cilt Enfeksiyonu", new List<string> { "Şişlik", "Ağrı", "Kızarıklık", "İltihap", "Deri Döküntüsü" } },
                { "Cilt Kanseri", new List<string> { "Şişlik", "Cilt Lekeleri", "İltihap", "Deri Döküntüsü", "Cilt Kanseri Belirtileri" } },
                { "Yanık", new List<string> { "Şişlik", "Ağrı", "Kızarıklık", "Yanık", "İltihap" } },
                { "Estetik Sorunlar", new List<string> { "Asimetri", "Şişlik", "Cilt Lekeleri", "Morarma", "Yaralanma" } },
                { "Cilt Kistleri", new List<string> { "Şişlik", "Ağrı", "İltihap", "Cilt Lekeleri" } },
                { "Lipoatrofi", new List<string> { "Asimetri", "Şişlik", "Cilt Lekeleri" } },
                { "Nekroz", new List<string> { "Şişlik", "Ağrı", "İltihap", "Kızarıklık", "Deri Döküntüsü" } },
                { "Keloid", new List<string> { "Şişlik", "Cilt Lekeleri", "Ağrı" } },
                { "Baskı Yaraları", new List<string> { "Şişlik", "Ağrı", "Kızarıklık", "İltihap", "Deri Döküntüsü" } }
            };
