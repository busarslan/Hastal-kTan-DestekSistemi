...
{
    public partial class GenelCerrahi : Form
    {
        ...
        public GenelCerrahi()
        {
            ...
            this.Load += new EventHandler(GenelCerrahi_Load); // Form yüklenirken çağrılacak olayı ekle
            this.ClientSize = new Size(800, 450); // Başlangıç boyutlarını ayarla
            this.Resize += new EventHandler(GenelCerrahi_Resize); // Form yeniden boyutlandırıldığında çağrılacak olayı ekle
            InitializeLabel1();  // alt ana başlık (kulak burun boğaz) 
            this.userTC = UserInfo.TCNo;
        }

        private void GenelCerrahi_Load(object sender, EventArgs e)
        {
            try {
        ...
        private void GenelCerrahi_Resize(object sender, EventArgs e)
        {
            ResizeControls();
            RepositionControls();
            CustomMenu.AddMenu(this);
        ...
        private void RepositionControls()
        {
            ...
            int checkBoxLeft = (this.ClientSize.Width - chkAbdominalPain.Width) / 2;
            ...
            using (OleDbConnection conn = new OleDbConnection(connectionString))
            {
                try
                {
                    ...
                    using (OleDbCommand command = new OleDbCommand(query, conn))
                    {
                        ...
                        command.Parameters.AddWithValue("@uzmanlık", "GenelCerrahi");
        ...
        private string DetermineDiseases()
        {
            var symptoms = new Dictionary<string, bool>
            {
                ...
                { "İştah Kaybı", chkAppetiteLoss.Checked },
                { "Hafif Ateş", chkMildFever.Checked },
                { "Sarılık", chkJaundice.Checked },
                { "Mide Ekşimesi", chkHeartburn.Checked },
                { "Şişkinlik", chkBloating.Checked },
                { "Kilo Kaybı", chkWeightLoss.Checked },
                { "Rektal Kanama", chkRectalBleeding.Checked },
                { "Şişlik ve Ağrı (kasık vs.)", chkSwellingPain.Checked }
            };

            var diseases = new Dictionary<string, List<string>>
            {
                { "Apandisit", new List<string> { "Karın Ağrısı", "Bulantı ve Kusma", "İştah Kaybı", "Hafif Ateş" } },
                { "Safra kesesi taşları (Kolelitiazis)", new List<string> { "Karın Ağrısı", "Bulantı ve Kusma", "Sarılık" } },
                { "Mide ve bağırsak ülserleri", new List<string> { "Karın Ağrısı", "Mide Ekşimesi", "Şişkinlik", "Bulantı ve Kusma" } },
                { "Kolon kanseri", new List<string> { "Karın Ağrısı", "Kilo Kaybı", "Rektal Kanama" } },
                { "Fıtık", new List<string> { "Şişlik ve Ağrı (kasık vs.)" } }
            };
