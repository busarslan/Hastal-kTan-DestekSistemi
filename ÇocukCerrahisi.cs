...
{
    public partial class ÇocukCerrahisi : Form
    {
        ...
        public ÇocukCerrahisi()
        {
            ...
            this.Load += new EventHandler(ÇocukCerrahisi_Load); // Form yüklenirken çağrılacak olayı ekle
            this.ClientSize = new Size(800, 450); // Başlangıç boyutlarını ayarla
            this.Resize += new EventHandler(ÇocukCerrahisi_Resize); // Form yeniden boyutlandırıldığında çağrılacak olayı ekle
        ...
        private void ÇocukCerrahisi_Load(object sender, EventArgs e)
        {
            try {
        ...
        private void ÇocukCerrahisi_Resize(object sender, EventArgs e)
        {
            ResizeControls();
            RepositionControls();
            CustomMenu.AddMenu(this);
        ...
        private void btnSave_Click(object sender, EventArgs e)
        {
            ...
            using (OleDbConnection conn = new OleDbConnection(connectionString))
            {
                try
                {
                    ...
                    {
                        command.Parameters.AddWithValue("@uzmanlık", "Çocuk Cerrahisi");
        ...
        private string DetermineDiseases()
        {
            var symptoms = new Dictionary<string, bool>
            {
                ...
                { "İdrar Kaçırma", chkUrinaryIncontinence.Checked },
                { "İnguinal Herni", chkInguinalHernia.Checked },
                { "Fıtık", chkHernia.Checked },
                { "İdrar Retansiyonu", chkUrinaryRetention.Checked },
                { "İdrar Yolu Taşı", chkUrinaryStone.Checked },
            };

            var diseases = new Dictionary<string, List<string>>
            {
                { "Apandisit", new List<string> { "Karın Ağrısı", "Kusma", "İshal" } },
                { "İdrar Yolu Enfeksiyonu", new List<string> { "İdrar Yolu Enfeksiyonu", "İdrar Yolu Taşı" } },
                { "İnmemiş Testis", new List<string> { "İnguinal Herni", "Fıtık" } },
                { "Üretral Fistül", new List<string> { "İdrar Kaçırma", "İdrar Retansiyonu" } },
            };
