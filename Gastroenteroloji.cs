...
using static Hastalik_Tani_Destek_Sistemi.Program;


namespace Hastalik_Tani_Destek_Sistemi
{
    public partial class Gastroenteroloji : Form
    {
        ...
        public Gastroenteroloji()
        {
            ...
            this.Load += new EventHandler(Gastroenteroloji_Load); // Form yüklenirken çağrılacak olayı ekle
            this.ClientSize = new Size(800, 450); // Başlangıç boyutlarını ayarla
            this.Resize += new EventHandler(Gastroenteroloji_Resize); // Form yeniden boyutlandırıldığında çağrılacak olayı ekle
            InitializeLabel1();  // alt ana başlık (kulak burun boğaz) 
            this.userTC = UserInfo.TCNo;
        }

        private void Gastroenteroloji_Load(object sender, EventArgs e)
        {
            try {
        ...
        private void Gastroenteroloji_Resize(object sender, EventArgs e)
        {
            ResizeControls();
            RepositionControls();
            CustomMenu.AddMenu(this);
        ...
        private void RepositionControls()
        {
            ...
            int checkBoxLeft = (this.ClientSize.Width - chkHeartburn.Width) / 2;
            ...
            using (OleDbConnection conn = new OleDbConnection(connectionString))
            {
                try
                {
                    ...
                    using (OleDbCommand command = new OleDbCommand(query, conn))
                    {
                        ...
                        command.Parameters.AddWithValue("@uzmanlık", "Gastroenteroloji");
        ...
        private string DetermineDiseases()
        {
            ...
            {
                { "Mide Ekşimesi", chkHeartburn.Checked },
                { "Halsizlik", chkFatigue.Checked },
                { "Karın Ağrısı", chkAbdominalPain.Checked },
                { "Bulantı", chkNausea.Checked },
                { "Kusma", chkVomiting.Checked },
                { "İshal", chkDiarrhea.Checked },
                { "Kabızlık", chkConstipation.Checked },
                { "Safra Kesesi Bölgesinde Ağrı", chkGallbladderPain.Checked },
                { "İştahsızlık", chkAppetiteLoss.Checked },
                { "Hızlı Kilometre Alımı", chkRapidWeightGain.Checked },
                { "Karın Şişkinliği", chkAbdominalBloating.Checked },
                { "Safra Kesesi Taşlarının Belirtileri", chkGallstonesSymptoms.Checked },
                { "Sindirim Sorunları", chkDigestiveIssues.Checked },
                { "Karın Gerginliği", chkAbdominalTightness.Checked }
            };

            var diseases = new Dictionary<string, List<string>>
            {
                { "Gastroözofageal Reflü Hastalığı", new List<string> { "Mide Ekşimesi", "Karın Ağrısı" } },
                { "Peptik Ülser Hastalığı", new List<string> { "Mide Ekşimesi", "Karın Ağrısı", "Bulantı" } },
                { "Kronik İnflamatuar Barsak Hastalığı", new List<string> { "Karın Ağrısı", "İshal", "Kabızlık" } },
                { "İrritabl Barsak Sendromu", new List<string> { "Karın Ağrısı", "İshal", "Kabızlık" } },
                { "Gastroenterit", new List<string> { "Bulantı", "Kusma", "İshal" } },
                { "Hepatit", new List<string> { "Halsizlik", "İştahsızlık", "Safra Kesesi Bölgesinde Ağrı" } },
                { "Safra Kesesi Hastalıkları", new List<string> { "Safra Kesesi Taşlarının Belirtileri", "Safra Kesesi Bölgesinde Ağrı" } }
            };
