...
{
    public partial class Onkoloji : Form
    {
        ...
        public Onkoloji()
        {
            ...
            this.Load += new EventHandler(Onkoloji_Load); // Form yüklenirken çağrılacak olayı ekle
            this.ClientSize = new Size(800, 450); // Başlangıç boyutlarını ayarla
            this.Resize += new EventHandler(Onkoloji_Resize); // Form yeniden boyutlandırıldığında çağrılacak olayı ekle
        ...
        private void Onkoloji_Load(object sender, EventArgs e)
        {
            try {
                ResizeControls();
                RepositionControls();
        ...
        private void Onkoloji_Resize(object sender, EventArgs e)
        {
            ResizeControls();
            RepositionControls();
            CustomMenu.AddMenu(this);
        ...
        private void RepositionControls()
        {
            ...
            int checkBoxLeft = (this.ClientSize.Width - chkWeightLoss.Width) / 2;
            ...
            using (OleDbConnection conn = new OleDbConnection(connectionString))
            {
                try
                {
                    ...
                    using (OleDbCommand command = new OleDbCommand(query, conn))
                    {
                        ...
                        command.Parameters.AddWithValue("@uzmanlık", "Onkoloji");
        ...
        private string DetermineDiseases()
        {
            var symptoms = new Dictionary<string, bool>
            {
                ...
                { "Şişlik veya yumru", chkSwelling.Checked },
                { "Cilt değişiklikleri", chkSkinChanges.Checked },
                { "Kanama veya morarma", chkBleeding.Checked },
                { "Ateş", chkFever.Checked },
                { "Gece terlemeleri", chkNightSweats.Checked },
                { "Sürekli öksürük", chkPersistentCough.Checked },
                { "Nefes darlığı", chkShortnessOfBreath.Checked },
                { "Bağırsak alışkanlıklarında değişiklik", chkBowelChanges.Checked },
                { "İdrar değişiklikleri", chkUrineChanges.Checked }
            };

            var diseases = new Dictionary<string, List<string>>
            {
                { "Akciğer Kanseri", new List<string> { "Sürekli öksürük", "Nefes darlığı", "Kilo kaybı" } },
                { "Meme Kanseri", new List<string> { "Şişlik veya yumru", "Cilt değişiklikleri", "Ağrı" } },
                { "Kolorektal Kanser", new List<string> { "Bağırsak alışkanlıklarında değişiklik", "Kanama", "Kilo kaybı" } },
                { "Prostat Kanseri", new List<string> { "İdrar değişiklikleri", "Ağrı", "Yorgunluk" } },
                { "Lösemi", new List<string> { "Yorgunluk", "Ateş", "Kanama veya morarma" } },
                { "Lenfoma", new List<string> { "Şişlik veya yumru", "Gece terlemeleri", "İştah kaybı" } }
            };
