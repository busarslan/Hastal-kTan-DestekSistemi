...
{
    public partial class EndokrinolojiveMetabolizma : Form
    {
        ...
        public EndokrinolojiveMetabolizma()
        {
            ...
            this.Load += new EventHandler(EndokrinolojiveMetabolizma_Load); // Form yüklenirken çağrılacak olayı ekle
            this.ClientSize = new Size(800, 450); // Başlangıç boyutlarını ayarla
            this.Resize += new EventHandler(EndokrinolojiveMetabolizma_Resize); // Form yeniden boyutlandırıldığında çağrılacak olayı ekle
        ...
        private void EndokrinolojiveMetabolizma_Load(object sender, EventArgs e)
        {
            try {
        ...
        private void EndokrinolojiveMetabolizma_Resize(object sender, EventArgs e)
        {
            ResizeControls();
            RepositionControls();
            CustomMenu.AddMenu(this);
        ...
        {
            baslik = new Label();
            baslik.Text = "Endokrinoloji ve Metabolizma";
        ...
        private void RepositionControls()
        {
            ...
            int checkBoxLeft = (this.ClientSize.Width - chkFatigue.Width) / 2;
            ...
            using (OleDbConnection conn = new OleDbConnection(connectionString))
            {
                try
                {
                    ...
                    using (OleDbCommand command = new OleDbCommand(query, conn))
                    {
                        ...
                        command.Parameters.AddWithValue("@uzmanlık", "EndokrinolojiveMetabolizma");
        ...
        private string DetermineDiseases()
        {
            ...
            {
                { "Yorgunluk", chkFatigue.Checked },
                { "Kilo alımı", chkWeightGain.Checked },
                { "Kilo kaybı", chkWeightLoss.Checked },
                { "Susuzluk hissi", chkThirst.Checked },
                { "Sık idrara çıkma", chkFrequentUrination.Checked },
                { "Ellerde veya ayaklarda uyuşma veya karıncalanma", chkNumbness.Checked },
                { "Soğuk intoleransı", chkColdIntolerance.Checked },
                { "Sıcak intoleransı", chkHeatIntolerance.Checked },
                { "Saç dökülmesi", chkHairLoss.Checked },
                { "Kalp çarpıntısı", chkPalpitations.Checked },
                { "Depresyon veya sinirlilik", chkDepression.Checked },
                { "Ciltte kuruluk veya koyulaşma", chkSkinChanges.Checked }
            };

            var diseases = new Dictionary<string, List<string>>
            {
                { "Diabetes Mellitus (Şeker Hastalığı)", new List<string> { "Kilo kaybı", "Susuzluk hissi", "Sık idrara çıkma" } },
                { "Hipotiroidi", new List<string> { "Yorgunluk", "Kilo alımı", "Soğuk intoleransı" } },
                { "Hipertiroidi", new List<string> { "Kilo kaybı", "Kalp çarpıntısı", "Sıcak intoleransı" } },
                { "Cushing Sendromu", new List<string> { "Kilo alımı", "Ciltte koyulaşma", "Depresyon" } },
                { "Addison Hastalığı", new List<string> { "Yorgunluk", "Kilo kaybı", "Ciltte koyulaşma" } }
            };
