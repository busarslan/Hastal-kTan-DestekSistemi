...
{
    public partial class Romatoloji : Form
    {
        ...
        public Romatoloji()
        {
            ...
            this.Load += new EventHandler(Romatoloji_Load); // Form yüklenirken çağrılacak olayı ekle
            this.ClientSize = new Size(800, 450); // Başlangıç boyutlarını ayarla
            this.Resize += new EventHandler(Romatoloji_Resize); // Form yeniden boyutlandırıldığında çağrılacak olayı ekle
        ...
        private void Romatoloji_Load(object sender, EventArgs e)
        {
            try {
        ...
        private void Romatoloji_Resize(object sender, EventArgs e)
        {
            ResizeControls();
            RepositionControls();
            CustomMenu.AddMenu(this);
        ...
        private void RepositionControls()
        {
            ...
            int checkBoxLeft = (this.ClientSize.Width - chkJointPain.Width) / 2;
            ...
            using (OleDbConnection conn = new OleDbConnection(connectionString))
            {
                try
                {
                    ...
                    using (OleDbCommand command = new OleDbCommand(query, conn))
                    {
                        ...
                        command.Parameters.AddWithValue("@uzmanlık", "Romatoloji");
        ...
        private string DetermineDiseases()
        {
            ...
            {
                { "Eklem ağrısı", chkJointPain.Checked },
                { "Eklem şişliği", chkJointSwelling.Checked },
                { "Eklem sertliği", chkJointStiffness.Checked },
                { "Kas ağrısı", chkMusclePain.Checked },
                { "Yorgunluk", chkFatigue.Checked },
                { "Sabah tutukluğu", chkMorningStiffness.Checked },
                { "Deri döküntüsü", chkSkinRash.Checked },
                { "Ateş", chkFever.Checked },
                { "Kilo kaybı", chkWeightLoss.Checked },
                { "Göz kuruluğu", chkDryEyes.Checked },
                { "Ağız kuruluğu", chkDryMouth.Checked },
                { "Raynaud fenomeni", chkRaynaudPhenomenon.Checked }
            };

            var diseases = new Dictionary<string, List<string>>
            {
                { "Romatoid Artrit", new List<string> { "Eklem ağrısı", "Eklem şişliği", "Sabah tutukluğu" } },
                { "Sistemik Lupus Eritematozus", new List<string> { "Eklem ağrısı", "Deri döküntüsü", "Yorgunluk" } },
                { "Ankilozan Spondilit", new List<string> { "Eklem ağrısı", "Sabah tutukluğu", "Kas ağrısı" } },
                { "Sjögren Sendromu", new List<string> { "Göz kuruluğu", "Ağız kuruluğu", "Eklem ağrısı" } },
                { "Sistemik Skleroz (Skleroderma)", new List<string> { "Deri sertleşmesi", "Raynaud fenomeni", "Eklem ağrısı" } }
            };
