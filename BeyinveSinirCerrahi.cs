...
{
    public partial class BeyinveSinirCerrahi : Form
    {
        ...
        public BeyinveSinirCerrahi()
        {
            ...
            this.Load += new EventHandler(BeyinveSinirCerrahi_Load); // Form yüklenirken çağrılacak olayı ekle
            this.ClientSize = new Size(800, 450); // Başlangıç boyutlarını ayarla
            this.Resize += new EventHandler(BeyinveSinirCerrahi_Resize); // Form yeniden boyutlandırıldığında çağrılacak olayı ekle
            InitializeLabel1();  // alt ana başlık (kulak burun boğaz) 
           
            this.userTC = UserInfo.TCNo;
        }

        private void BeyinveSinirCerrahi_Load(object sender, EventArgs e)
        {
            try
            {
                ResizeControls();
        ...
        private void BeyinveSinirCerrahi_Resize(object sender, EventArgs e)
        {
            ResizeControls();
            RepositionControls();
            CustomMenu.AddMenu(this);
        ...
        {
            baslik = new Label();
            baslik.Text = "Beyin ve Sinir Cerrahi";
        ...
        private void RepositionControls()
        {
            ...
            int checkBoxLeft = (this.ClientSize.Width - chkHeadache.Width) / 2;
    ...
    foreach (Control control in this.Controls)
    {
            ...
            using (OleDbConnection conn = new OleDbConnection(connectionString))
            {
                try
                {
                    ...
                    using (OleDbCommand command = new OleDbCommand(query, conn))
                    {
                        ...
                        command.Parameters.AddWithValue("@uzmanlık", "BeyinveCerrahi");
        ...
        private string DetermineDiseases()
        {
            var symptoms = new Dictionary<string, bool>
            {
                ...
                { "Baş Dönmesi", chkDizziness.Checked },
                { "Bilinç Kaybı", chkLossOfConsciousness.Checked },
                { "Nöbetler", chkSeizures.Checked },
                { "Hareket Kaybı", chkLossOfMovement.Checked },
                { "Konuşma Güçlüğü", chkSpeechDifficulty.Checked },
                { "Hafıza Kaybı", chkMemoryLoss.Checked },
                { "Karıncalanma", chkNumbness.Checked },
                { "Görme Değişiklikleri", chkVisionChanges.Checked },
                { "Zayıflık", chkWeakness.Checked },
                { "Yürüme Zorluğu", chkDifficultyWalking.Checked },
                { "Denge Problemleri", chkBalanceProblems.Checked }
            };

            var diseases = new Dictionary<string, List<string>>
            {
                { "Beyin Tümörü", new List<string> { "Baş Ağrısı", "Bilinç Kaybı", "Nöbetler", "Konuşma Güçlüğü", "Hafıza Kaybı" } },
                { "Beyin Kanaması", new List<string> { "Baş Ağrısı", "Bilinç Kaybı", "Hareket Kaybı", "Nöbetler", "Karıncalanma" } },
                { "Beyin İnme", new List<string> { "Baş Ağrısı", "Baş Dönmesi", "Bilinç Kaybı", "Hareket Kaybı", "Konuşma Güçlüğü" } },
                { "Beyin Enfeksiyonu", new List<string> { "Baş Ağrısı", "Bilinç Kaybı", "Nöbetler", "Hafıza Kaybı", "Hareket Kaybı" } },
                { "Beyin Travması", new List<string> { "Baş Ağrısı", "Bilinç Kaybı", "Hareket Kaybı", "Nöbetler", "Karıncalanma" } }
            };
            ...
            {
                return "Belirtiler herhangi bir hastalıkla eşleşmiyor.";
            }
        }
    }
}