...
{
    public partial class Üroloji : Form
    {
        ...
        public Üroloji()
        {
            ...
            ResizeControls(); 
            this.Load += new EventHandler(Üroloji_Load); // Form yüklenirken çağrılacak olayı ekle
            this.ClientSize = new Size(800, 450); // Başlangıç boyutlarını ayarla
            this.Resize += new EventHandler(Üroloji_Resize); // Form yeniden boyutlandırıldığında çağrılacak olayı ekle
        ...
        private void Üroloji_Load(object sender, EventArgs e)
        {
            try {
        ...
        private void Üroloji_Resize(object sender, EventArgs e)
        {
            ResizeControls();
            RepositionControls();
            CustomMenu.AddMenu(this);
        ...
        private void RepositionControls()
        {
            ...
            int checkBoxLeft = (this.ClientSize.Width - chkUrinaryTractInfection.Width) / 2;
        ...
        private void btnSave_Click(object sender, EventArgs e)
        {
            ...
            using (OleDbConnection conn = new OleDbConnection(connectionString))
            {
                try
                {
                    ...
                    {
                        command.Parameters.AddWithValue("@uzmanlık", "Üroloji");
        ...
        private string DetermineDiseases()
        {
            ...
            {
                { "İdrar Yolu Enfeksiyonu", chkUrinaryTractInfection.Checked },
                { "İdrar Kaçırma", chkIncontinence.Checked },
                { "Kanlı İdrar", chkBloodyUrine.Checked },
                { "İdrarda Yanma", chkBurningDuringUrination.Checked },
                { "Böbrek Ağrısı", chkKidneyPain.Checked },
                { "İdrar Hızında Azalma", chkReducedUrination.Checked },
                { "Cinsel İşlev Bozukluğu", chkSexualDysfunction.Checked },
                { "Genital Ağrı", chkGenitalPain.Checked },
                { "Testis Ağrısı", chkTesticularPain.Checked },
                { "Mesane İşlev Bozukluğu", chkBladderDysfunction.Checked }
            };

            var diseases = new Dictionary<string, List<string>>
            {
                { "İdrar Yolu Enfeksiyonu", new List<string> { "İdrar Yolu Enfeksiyonu", "İdrarda Yanma", "İdrar Kaçırma", "Kanlı İdrar" } },
                { "BPH (Benign Prostat Hiperplazisi)", new List<string> { "İdrar Hızında Azalma", "Böbrek Ağrısı", "İdrar Kaçırma", "Kanlı İdrar" } },
                { "Prostat Kanseri", new List<string> { "İdrar Hızında Azalma", "İdrarda Yanma", "İdrar Kaçırma", "Genital Ağrı" } },
                { "Testis Kanseri", new List<string> { "Testis Ağrısı", "Genital Ağrı" } },
                { "Üretral Darlık", new List<string> { "İdrar Kaçırma", "İdrarda Yanma", "Genital Ağrı" } },
                { "Üriner Taş", new List<string> { "İdrar Yolu Enfeksiyonu", "Böbrek Ağrısı", "Kanlı İdrar" } }
            };
