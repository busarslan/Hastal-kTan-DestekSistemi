...
{
    public partial class Dermatoloji : Form
    {
        ...
        public Dermatoloji()
        {
            ...
            this.Load += new EventHandler(Dermatoloji_Load); // Form yüklenirken çağrılacak olayı ekle
            this.ClientSize = new Size(800, 450); // Başlangıç boyutlarını ayarla
            this.Resize += new EventHandler(Dermatoloji_Resize); // Form yeniden boyutlandırıldığında çağrılacak olayı ekle
            InitializeLabel1();  // alt ana başlık (kulak burun boğaz) 
            this.userTC = UserInfo.TCNo;
        }

        private void Dermatoloji_Load(object sender, EventArgs e)
        {
            try {
        ...
        private void Dermatoloji_Resize(object sender, EventArgs e)
        {
            ResizeControls();
            RepositionControls();
            CustomMenu.AddMenu(this);
        ...
        private void RepositionControls()
        {
            ...
            int checkBoxLeft = (this.ClientSize.Width - chkItching.Width) / 2;
            ...
            using (OleDbConnection conn = new OleDbConnection(connectionString))
            {
                try
                {
                    ...
                    using (OleDbCommand command = new OleDbCommand(query, conn))
                    {
                        ...
                        command.Parameters.AddWithValue("@uzmanlık", "Dermatoloji");
        ...
        private string DetermineDiseases()
        {
            ...
            {
                { "Kaşıntı", chkItching.Checked },
                { "Döküntü", chkRash.Checked },
                { "Kızarıklık", chkRedness.Checked },
                { "Kabarma", chkSwelling.Checked },
                { "Yara", chkSore.Checked },
                { "Pullanma", chkFlaking.Checked },
                { "Kuruluk", chkDryness.Checked },
                { "Yanma", chkBurning.Checked },
                { "Leke", chkDiscoloration.Checked },
                { "Köpüklenme", chkFrothing.Checked }
            };

            var diseases = new Dictionary<string, List<string>>
            {
                { "Egzama", new List<string> { "Kaşıntı", "Döküntü", "Kızarıklık" } },
                { "Sedef Hastalığı", new List<string> { "Pullanma", "Kızarıklık", "Kabarma" } },
                { "Alerjik Dermatit", new List<string> { "Kaşıntı", "Kızarıklık", "Döküntü" } },
                { "Akne", new List<string> { "Sivilce", "Kızarıklık", "Leke" } },
                { "Rosacea", new List<string> { "Kızarıklık", "Yanma", "Köpüklenme" } },
                { "Psoriasis", new List<string> { "Pullanma", "Kızarıklık", "Kabarma" } },
                { "Kontakt Dermatit", new List<string> { "Kaşıntı", "Kızarıklık", "Leke" } },
                { "Kuruluk", new List<string> { "Kuruluk", "Pullanma" } },
                { "İltihaplı Sivilce", new List<string> { "Sivilce", "Kızarıklık", "Yara" } },
                { "Vitiligo", new List<string> { "Leke", "Discoloration" } }
            };
