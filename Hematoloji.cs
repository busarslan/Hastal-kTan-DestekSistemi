...
{
    public partial class Hematoloji : Form
    {
        ...
        public Hematoloji()
        {
            ...
            this.Load += new EventHandler(Hematoloji_Load); // Form yüklenirken çağrılacak olayı ekle
            this.ClientSize = new Size(800, 450); // Başlangıç boyutlarını ayarla
            this.Resize += new EventHandler(Hematoloji_Resize); // Form yeniden boyutlandırıldığında çağrılacak olayı ekle
            InitializeLabel1();  // alt ana başlık (kulak burun boğaz) 
            this.userTC = UserInfo.TCNo;
        }

        private void Hematoloji_Load(object sender, EventArgs e)
        {
            try
            {
        ...
        private void Hematoloji_Resize(object sender, EventArgs e)
        {
        ...
        private void CreateButtonBck()  // geri butonu
        {
            ...
            buttonBck.Text = " ← ";
            this.buttonBck.ForeColor = Color.White;
        ...
        private void RepositionControls()
        {
            ...
            int checkBoxLeft = (this.ClientSize.Width - chkAnemia.Width) / 2;
            ...
            using (OleDbConnection conn = new OleDbConnection(connectionString))
            {
                try
                {
                    ...
                    using (OleDbCommand command = new OleDbCommand(query, conn))
                    {
                        ...
                        command.Parameters.AddWithValue("@uzmanlık", "Hematoloji");
        ...
        private string DetermineDiseases()
        {
            ...
            {
                { "Anemi (kansızlık)", chkAnemia.Checked },
                { "Yorgunluk", chkFatigue.Checked },
                { "Halsizlik", chkWeakness.Checked },
                { "Soluk cilt", chkPaleSkin.Checked },
                { "Nefes darlığı", chkShortnessOfBreath.Checked },
                { "Baş dönmesi", chkDizziness.Checked },
                { "Çabuk morarma", chkEasyBruising.Checked },
                { "Kanama eğilimi", chkBleedingTendency.Checked },
                { "Ateş", chkFever.Checked },
                { "Gece terlemeleri", chkNightSweats.Checked },
                { "Kilo kaybı", chkWeightLoss.Checked },
                { "Lenfadenopati (lenf bezi şişliği)", chkLymphadenopathy.Checked },
                { "Kemik ağrısı", chkBonePain.Checked }
            };

            var diseases = new Dictionary<string, List<string>>
            {
                { "Demir Eksikliği Anemisi", new List<string> { "Anemi (kansızlık)", "Yorgunluk", "Soluk cilt" } },
                { "Akut Lenfoblastik Lösemi", new List<string> { "Ateş", "Gece terlemeleri", "Çabuk morarma" } },
                { "Kronik Lenfositik Lösemi", new List<string> { "Lenfadenopati", "Yorgunluk", "Kilo kaybı" } },
                { "Hodgkin Lenfoma", new List<string> { "Lenfadenopati", "Gece terlemeleri", "Ateş" } },
                { "Non-Hodgkin Lenfoma", new List<string> { "Lenfadenopati", "Kilo kaybı", "Gece terlemeleri" } },
                { "Multipl Miyelom", new List<string> { "Kemik ağrısı", "Yorgunluk", "Anemi (kansızlık)" } }
            };
