...
{
    public partial class Nefroloji : Form
    {
        ...
        public Nefroloji()
        {
            ...
            this.Load += new EventHandler(Nefroloji_Load); // Form yüklenirken çağrılacak olayı ekle
            this.ClientSize = new Size(800, 450); // Başlangıç boyutlarını ayarla
            this.Resize += new EventHandler(Nefroloji_Resize); // Form yeniden boyutlandırıldığında çağrılacak olayı ekle
        ...
        private void Nefroloji_Load(object sender, EventArgs e)
        {
            try {
        ...
        private void Nefroloji_Resize(object sender, EventArgs e)
        {
            ResizeControls();
            RepositionControls();
            CustomMenu.AddMenu(this);
        ...
        private void RepositionControls()
        {
            ...
            int checkBoxLeft = (this.ClientSize.Width - chkDecreasedUrination.Width) / 2;
            ...
            using (OleDbConnection conn = new OleDbConnection(connectionString))
            {
                try
                {
                    ...
                    using (OleDbCommand command = new OleDbCommand(query, conn))
                    {
                        ...
                        command.Parameters.AddWithValue("@uzmanlık", "Nefroloji");
        ...
        private string DetermineDiseases()
        {
            ...
            {
                { "İdrar miktarında azalma", chkDecreasedUrination.Checked },
                { "İdrar renginde değişiklik", chkChangeInUrineColor.Checked },
                { "İdrarda kan görülmesi", chkBloodInUrine.Checked },
                { "Yorgunluk ve halsizlik", chkFatigue.Checked },
                { "Ani kilo değişiklikleri", chkSuddenWeightChanges.Checked },
                { "Sırt ve kasık bölgesinde ağrı", chkBackGroinPain.Checked },
                { "Yüksek kan basıncı", chkHighBloodPressure.Checked },
                { "Ödem (Şişlik)", chkEdema.Checked },
                { "Bulantı ve kusma", chkNauseaVomiting.Checked }
            };

            var diseases = new Dictionary<string, List<string>>
            {
                { "Böbrek taşı", new List<string> { "İdrar miktarında azalma", "İdrar renginde değişiklik", "Sırt ve kasık bölgesinde ağrı" } },
                { "Böbrek yetmezliği", new List<string> { "İdrar miktarında azalma", "Yorgunluk ve halsizlik", "Ödem (şişlik)" } },
                { "İdrar yolu enfeksiyonları", new List<string> { "İdrarda kan görülmesi", "Yüksek kan basıncı", "Bulantı ve kusma" } },
                { "Glomerülonefrit", new List<string> { "İdrar renginde değişiklik", "Yüksek kan basıncı", "Ödem (şişlik)" } },
                { "Polikistik Böbrek Hastalığı", new List<string> { "İdrar miktarında azalma", "Sırt ve kasık bölgesinde ağrı", "Bulantı ve kusma" } },
                { "Hipertansif Nefropati", new List<string> { "Yüksek kan basıncı", "İdrar miktarında azalma", "Ödem (şişlik)" } },
                { "Nefrotik Sendrom", new List<string> { "İdrar renginde değişiklik", "Ödem (şişlik)", "Yorgunluk ve halsizlik" } }
            };
