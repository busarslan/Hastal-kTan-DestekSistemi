...
{
    public partial class Kardiyoloji : Form
    {
        ...
        public Kardiyoloji()
        {
            ...
            this.Load += new EventHandler(Kardiyoloji_Load); // Form yüklenirken çağrılacak olayı ekle
            this.ClientSize = new Size(800, 450); // Başlangıç boyutlarını ayarla
            this.Resize += new EventHandler(Kardiyoloji_Resize); // Form yeniden boyutlandırıldığında çağrılacak olayı ekle
        ...
        private void Kardiyoloji_Load(object sender, EventArgs e)
        {
            try {
        ...
        private void Kardiyoloji_Resize(object sender, EventArgs e)
        {
            ResizeControls();
            RepositionControls();
            CustomMenu.AddMenu(this);
            ...
            using (OleDbConnection conn = new OleDbConnection(connectionString))
            {
                try
                {
                    ...
                    using (OleDbCommand command = new OleDbCommand(query, conn))
                    {
                        ...
                        command.Parameters.AddWithValue("@uzmanlık", "Kardiyoloji");
        ...
        private string DetermineDiseases()
        {
            var symptoms = new Dictionary<string, bool>
            {
                ...
                { "Çarpıntı", chkPalpitations.Checked },
                { "Yorgunluk", chkFatigue.Checked },
                { "Bayılma", chkFainting.Checked },
                { "Ödem", chkEdema.Checked },
                { "Bacak Ağrısı", chkLegPain.Checked },
                { "Çabuk Yorulma", chkEasyFatigue.Checked }
            };

            var diseases = new Dictionary<string, List<string>>
            {
                { "Koroner Arter Hastalığı", new List<string> { "Göğüs Ağrısı", "Nefes Darlığı", "Çarpıntı" } },
                { "Kalp Yetmezliği", new List<string> { "Nefes Darlığı", "Yorgunluk", "Ödem" } },
                { "Aritmi", new List<string> { "Çarpıntı", "Bayılma" } },
                { "Damar Tıkanıklığı", new List<string> { "Bacak Ağrısı", "Çabuk Yorulma" } }
            };

            var selectedSymptoms = symptoms.Where(s => s.Value).Select(s => s.Key).ToList();
            if (selectedSymptoms.Count < 2)
            {
                return "Belirtiler yetersiz, lütfen en az 2 belirti seçin.";
            }
