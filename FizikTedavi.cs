...
namespace Hastalik_Tani_Destek_Sistemi
{
    ...
    public partial class FizikTedavi : Form
    {
        ...
        public FizikTedavi()
        {
            ...
            this.Load += new EventHandler(FizikTedavi_Load); // Form yüklenirken çağrılacak olayı ekle
            this.ClientSize = new Size(800, 450); // Başlangıç boyutlarını ayarla
            this.Resize += new EventHandler(FizikTedavi_Resize); // Form yeniden boyutlandırıldığında çağrılacak olayı ekle
            InitializeLabel1();  // alt ana başlık (kulak burun boğaz) 
            this.userTC = UserInfo.TCNo;
        }

        private void FizikTedavi_Load(object sender, EventArgs e)
        {
            try {
                ResizeControls();
                RepositionControls();
        ...
        private void FizikTedavi_Resize(object sender, EventArgs e)
        {
            ResizeControls();
            RepositionControls();
            CustomMenu.AddMenu(this);
        ...
        {
            baslik = new Label();
            baslik.Text = "Fizik Tedavi ve Rehabilitasyon";
        ...
        private void RepositionControls()
        {
            ...
            int checkBoxLeft = (this.ClientSize.Width - chkPain.Width) / 2;
            ...
            using (OleDbConnection conn = new OleDbConnection(connectionString))
            {
                try
                {
                    ...
                    using (OleDbCommand command = new OleDbCommand(query, conn))
                    {
                           ...
                           command.Parameters.AddWithValue("@uzmanlık", "FizikTedavi");
        ...
        private string DetermineDiseases()
        {
            var symptoms = new Dictionary<string, bool>
            {
                ...
                { "Esneklik Kaybı", chkFlexibilityLoss.Checked },
                { "Zayıflık", chkWeakness.Checked },
                { "Kas İncinmesi", chkMuscleStrain.Checked },
                { "Eklem Sertliği", chkJointStiffness.Checked },
                { "Hareket Kısıtlılığı", chkMobilityLimitation.Checked },
                { "Kasılmalar", chkSpasms.Checked },
                { "Yorgunluk", chkFatigue.Checked },
                { "Denge Sorunları", chkBalanceIssues.Checked },
                { "Postür Bozukluğu", chkPosturalIssues.Checked }
            };

            var diseases = new Dictionary<string, List<string>>
            {
                { "Bel Ağrısı", new List<string> { "Ağrı", "Esneklik Kaybı" } },
                { "Omuz Yaralanması", new List<string> { "Ağrı", "Zayıflık", "Kas İncinmesi" } },
                { "Diz Osteoartriti", new List<string> { "Ağrı", "Eklem Sertliği", "Hareket Kısıtlılığı" } },
                { "Fibromiyalji", new List<string> { "Ağrı", "Yorgunluk", "Kasılmalar" } },
                { "Karpal Tünel Sendromu", new List<string> { "Ağrı", "Zayıflık", "Denge Sorunları" } },
                { "Skolyoz", new List<string> { "Ağrı", "Postür Bozukluğu", "Esneklik Kaybı" } },
                { "Tendinit", new List<string> { "Ağrı", "Esneklik Kaybı", "Kas İncinmesi" } },
                { "Kas Yırtılması", new List<string> { "Ağrı", "Zayıflık", "Kas İncinmesi" } },
                { "Omurga Daralması", new List<string> { "Ağrı", "Esneklik Kaybı", "Postür Bozukluğu" } },
                { "Burkulma", new List<string> { "Ağrı", "Esneklik Kaybı", "Zayıflık" } }
            };
